...
const initialState = {
  cartItems:
    JSON.parse(localStorage.getItem("cart")) || new Array(24).fill(null),
  total:
...
const cartSlice = createSlice({
  ...
  reducers: {
    clearCart: (state) => {
      state.cartItems = new Array(24).fill(null);
      localStorage.setItem("cart", JSON.stringify(new Array(24).fill(null)));
      state.amount = 0;
      state.total = 0;
    },
